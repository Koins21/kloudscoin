...
// Copyright (c) 2017-2019 The kloudcoins developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/kloudcoins-config.h"
#endif
...
#if ENABLE_ZMQ
#include "zmq/zmqnotificatiokloudcoinserface.h"
#endif
...
#if ENABLE_ZMQ
static CZMQNotificatiokloudcoinserface* pzmqNotificatiokloudcoinserface = NULL;
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("kloudcoins-shutoff");
...
#if ENABLE_ZMQ
    if (pzmqNotificatiokloudcoinserface) {
        Unregistervalidationinterface(pzmqNotificatiokloudcoinserface);
        delete pzmqNotificatiokloudcoinserface;
        pzmqNotificatiokloudcoinserface = NULL;
    }
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "kloudcoins.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "kloudcoinsd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the kloudcoins and zECA money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in kloudcoins/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in kloudcoins/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, kloudcoins, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in kloudcoins/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all kloudcoins specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-ecastake=<n>", strprintf(_("Enable or disable staking functionality for kloudcoins inputs (0-1, default: %u)"), 1));
...
//    strUsage += "  -anonymizekloudcoinsamount=<n>     " + strprintf(_("Keep N kloudcoins anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The kloudcoins Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The kloudcoins Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("kloudcoins-loadblk");
...
/** Sanity checks
 *  Ensure that kloudcoins is running in a usable environment with all
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. kloudcoins is shutting down."));
...
#endif
    // Make sure only a single kloudcoins process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. kloudcoins is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("kloudcoins version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
#if ENABLE_ZMQ
    pzmqNotificatiokloudcoinserface = CZMQNotificatiokloudcoinserface::CreateWithArguments(mapArgs);

    if (pzmqNotificatiokloudcoinserface) {
        Registervalidationinterface(pzmqNotificatiokloudcoinserface);
    }
#endif

    // ********************************************************* Step 7: load block chain

    //kloudcoins: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            ...
            try {
                ...
                //kloudcoins specific: zerocoin and spork DB's
                ...
                // kloudcoins: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // kloudcoins: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of kloudcoins") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart kloudcoins to complete") << "\n";
...
//    }
//
//    nAnonymizekloudcoinsAmount = GetArg("-anonymizekloudcoinsamount", 0);
//    if (nAnonymizekloudcoinsAmount > 999999) nAnonymizekloudcoinsAmount = 999999;
//    if (nAnonymizekloudcoinsAmount < 2) nAnonymizekloudcoinsAmount = 2;
    ...
    LogPrintf("Anonymize kloudcoins Amount %d\n", nAnonymizekloudcoinsAmount);
