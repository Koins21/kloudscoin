...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        LogPrint("kloudcoins", "CreateNewBlock(): total size %u\n", nBlockSize);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("kloudcoinsMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("kloudcoinsMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("kloudcoinsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kloudcoins-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrint("kloudcoins", "Running kloudcoinsMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
