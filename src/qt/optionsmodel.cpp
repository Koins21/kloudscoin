...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::kloudcoins);
    ...
    if (!settings.contains("nAnonymizekloudcoinsAmount"))
        settings.setValue("nAnonymizekloudcoinsAmount", 1000);

    nAnonymizekloudcoinsAmount = settings.value("nAnonymizekloudcoinsAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizekloudcoinsAmount"))
        SoftSetArg("-anonymizekloudcoinsamount", settings.value("nAnonymizekloudcoinsAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in kloudcoins.cpp during shotdown to also remove the window positions
...
//            return settings.value("fShowMasternodesTab");
        ...
        case AnonymizekloudcoinsAmount:
            return QVariant(nAnonymizekloudcoinsAmount);
        case Listen:
...
//            break;
        ...
        case AnonymizekloudcoinsAmount:
            nAnonymizekloudcoinsAmount = value.toInt();
            settings.setValue("nAnonymizekloudcoinsAmount", nAnonymizekloudcoinsAmount);
            emit anonymizekloudcoinsAmountChanged(nAnonymizekloudcoinsAmount);
            break;
        case CoinControlFeatures:
