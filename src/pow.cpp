...
{
    /* current difficulty formula, kloudcoins - DarkGravity v3, written by Evan Duffield - evan@dashpay.io */
...
unsigned int GetLegacyNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader* pblock, bool fProofOfStake)
{
    ...
    // kloudcoins coin: target change every block
    // kloudcoins coin: retarget with exponential moving toward target spacing
    uint256 bnNew;
    bnNew.SetCompact(pindexPrev->nBits);

    if (pindexLast->nHeight+1 >= nHardForkBlock)
    {
        int64_t kloudcoinserval = nTargetTimespan / nTargetSpacing;
        bnNew *= ((kloudcoinserval - 1) * nTargetSpacing + nActualSpacing + nActualSpacing);
        bnNew /= ((kloudcoinserval + 1) * nTargetSpacing);
    }
    else
    {
        int64_t kloudcoinserval = nTargetTimespan / nTargetSpacingOld;
        bnNew *= ((kloudcoinserval - 1) * nTargetSpacingOld + nActualSpacing + nActualSpacing);
        bnNew /= ((kloudcoinserval + 1) * nTargetSpacingOld);
    }
