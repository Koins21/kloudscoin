...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 kloudcoins at the same time
...
{
private:
    int GetDepthInMainChaikloudcoinsERNAL(const CBlockIndex*& pindexRet) const;
...
public:
    ...
    {
        const CBlockIndex* pindexRet;
        return GetDepthInMainChaikloudcoinsERNAL(pindexRet) > 0;
    }
...
public:
    ...
    int64_t nTime;
    std::string strOtkloudcoinsccount;
    std::string strComment;
    mapValue_t mapValue;
    int64_t nOrderPos; //! position in ordered transaction list
    uint64_t nEntryNo;
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion)
    {
        ...
        READWRITE(nTime);
        READWRITE(LIMITED_STRING(strOtkloudcoinsccount, 65536));
