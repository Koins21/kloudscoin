...
// Get time weight
int64_t GetWeight(int64_t kloudcoinservalBeginning, int64_t kloudcoinservalEnd)
{
    return min(kloudcoinservalEnd - kloudcoinservalBeginning - nStakeMinAge, (int64_t)nStakeMaxAge);
}
...
static int64_t GetStakeModifierSelectiokloudcoinservalSection(int nSection)
{
...
static int64_t GetStakeModifierSelectiokloudcoinserval()
{
    int64_t nSelectiokloudcoinserval = 0;
    for (int nSection = 0; nSection < 64; nSection++) {
        nSelectiokloudcoinserval += GetStakeModifierSelectiokloudcoinservalSection(nSection);
    }
    return nSelectiokloudcoinserval;
}

// select a block from the candidate blocks in vSortedByTimestamp, excluding
// already selected blocks in vSelectedBlocks, and with timestamp up to
// nSelectiokloudcoinservalStop.
static bool SelectBlockFromCandidates(
    vector<pair<int64_t, uint256> >& vSortedByTimestamp,
    map<uint256, const CBlockIndex*>& mapSelectedBlocks,
    int64_t nSelectiokloudcoinservalStop,
    ...
    const CBlockIndex** pindexSelected)
{
    ...
    *pindexSelected = (const CBlockIndex*)0;
    BOOST_FOREACH (const PAIRTYPE(int64_t, uint256) & item, vSortedByTimestamp) {
        ...
        if (fSelected && pindex->GetBlockTime() > nSelectiokloudcoinservalStop)
            break;
...
bool ComputeNextStakeModifier(const CBlockIndex* pindexPrev, uint64_t& nStakeModifier, bool& fGeneratedStakeModifier)
{
    ...
    int64_t nSelectiokloudcoinserval = GetStakeModifierSelectiokloudcoinserval();
    int64_t nSelectiokloudcoinservalStart = (pindexPrev->GetBlockTime() / getIntervalVersion(fTestNet)) * getIntervalVersion(fTestNet) - nSelectiokloudcoinserval;
    const CBlockIndex* pindex = pindexPrev;

    while (pindex && pindex->GetBlockTime() >= nSelectiokloudcoinservalStart) {
    ...
    int64_t nSelectiokloudcoinservalStop = nSelectiokloudcoinservalStart;
    map<uint256, const CBlockIndex*> mapSelectedBlocks;
    for (int nRound = 0; nRound < min(64, (int)vSortedByTimestamp.size()); nRound++) {
        // add an interval section to the current selection round
        nSelectiokloudcoinservalStop += GetStakeModifierSelectiokloudcoinservalSection(nRound);

        // select a block from the candidates of current round
        if (!SelectBlockFromCandidates(vSortedByTimestamp, mapSelectedBlocks, nSelectiokloudcoinservalStop, nStakeModifier, &pindex))
        ...
        if (fDebug || GetBoolArg("-printstakemodifier", false))
            LogPrintf("ComputeNextStakeModifier: selected round %d stop=%s height=%d bit=%d\n",
                nRound, DateTimeStrFormat("%Y-%m-%d %H:%M:%S", nSelectiokloudcoinservalStop).c_str(), pindex->nHeight, pindex->GetStakeEntropyBit());
    }
...
bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    int64_t nStakeModifierSelectiokloudcoinserval = GetStakeModifierSelectiokloudcoinserval();
    const CBlockIndex* pindex = pindexFrom;
    CBlockIndex* pindexNext = chainActive[pindexFrom->nHeight + 1];

    // loop to find the stake modifier later by a selection interval
    while (nStakeModifierTime < pindexFrom->GetBlockTime() + nStakeModifierSelectiokloudcoinserval) {
        if (!pindexNext) {
            // Should never happen
